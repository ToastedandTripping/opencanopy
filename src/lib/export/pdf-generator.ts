...
function buildReportHtml(options: ReportOptions): string {
  ...
  if (financial) {
    ...
    // to-the-dollar figure implies precision the inputs don't have (X4).
    // Round for display only; the underlying FinancialValue math is unchanged.
    ...
    const bars = roundedCarbonValues
      .map((cv, i) => {
    ...
    const stumpageW = Math.max((roundedStumpage / maxValue) * 100, 2);
    const stumpageBar = `
      <div style="margin-bottom:10px;">
        <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;">
          <span style="color:#555;">Logging revenue</span>
          <span style="color:#222;font-variant-numeric:tabular-nums;">${fmtCurrency(roundedStumpage)}</span>
        </div>
        <div style="height:10px;border-radius:6px;background:#f0f0f0;overflow:hidden;">
    ...
    const ecosystemHtml =
      roundedEcosystem > 0
        ? `<div style="margin-top:12px;padding:10px 14px;border-radius:8px;background:#f8fffe;border:1px solid #e0f2f1;">
            <span style="font-size:15px;color:#222;font-weight:600;">${fmtCurrency(roundedEcosystem)}</span>
...
<div style="margin-top:32px;padding-top:16px;border-top:1px solid #e5e5e5;font-size:10px;color:#aaa;line-height:1.6;">
  <p style="margin:0 0 4px 0;">Approximate estimate &mdash; may overestimate by up to ~20% (see methodology at opencanopy.ca). Not audited carbon accounting.</p>
