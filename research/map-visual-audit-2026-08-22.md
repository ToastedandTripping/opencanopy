...
|---|---|---|---|---|---|---|
| P1 | M | Deep-link camera silently dropped: on-mount flyTo polls for the map 20×100 ms then gives up. Any load >2.2 s (cold cache, mobile, every capture) lands on the default view with `layers=` still applied. Hits every share link and the landing CTA (`/map#STORY_END_CAMERA`). | `src/hooks/useMapState.ts:160–195` | Poll until the ref exists (10 Hz → 2 Hz, 120 s cap), fly once. No `load` re-fly: MapLibre honours an early flyTo and a late one would stop() a user pan. | 0 | shipped (Batch 0, `fix/map-deeplink-hydration`) |
| P8 | m | `preset=` alone restores nothing; restore is gated on `parsed.layers`. UI share links write both params and survive; hand-written/doc links do not. | `useMapState.ts:251` | Both paths resolve the preset (`parseLayersFromHash` on load; validated ids on popstate so an all-invalid `layers=` falls to the preset). | 0 | shipped (Batch 0) |
...
| P9 | m | Back/Forward restore (`handlePopState` → `onLayerRestore`) changes `layerKey`, so the push effect `pushState`s a new entry and discards Forward history. Pre-existing; found by Razor during Batch 0. | `useMapState.ts` push effect | Set a `restoring` ref in `handlePopState`; the push effect updates `lastLayerHash` without pushing while it is set. | 3 | open |
...
| T2 | M | Layer named "Old Growth Forest" shows 7–29 % of land; story's last number is 0.3 %. | Rename → "Stands 250+ yr (VRI)". Big-tree subset layer if `SITE_INDEX` survives in tiles (else data item). SITE_INDEX absent from current extract but EXISTS UPSTREAM in VRI WFS schema (DescribeFeatureType, 2026-08-27: `SITE_INDEX` number, nillable; `PROJ_HEIGHT_1` also present). Data pull feasible: add both to `transform.ts` extract + tile rebuild. | 1 (copy: done) / 4 (data) | copy done, data feasible (Batch 4) |
