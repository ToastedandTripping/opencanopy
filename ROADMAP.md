---
status: in-progress
current: "DEPLOYED to opencanopy.ca today (2026-06-20): a11y FAILs, Wave 3 styling, v3 raster paint-order (province z5 now reads RED/disturbance-dominant — the honest forest-loss view), and the story no longer using the forest-age overview as its base (green substrate + pre-1950 cuts, live-verified). NEXT BODY OF WORK = the scroll-story render rebuild + polish, scoped by a critic-gated deep research sweep (report: ~/marvin/research/scroll-driven-temporal-map-animation-20260620/). Consolidated 5-phase plan ready; awaiting Lee's Phase-3 decision (jump straight to the year-encoded-raster + custom-WebGL decode shader [~8-10d, the GFW technique] vs a frames stopgap [~2-3d])."
next: "Consolidated plan (consolidated-plan.md in the research dir): Phase 1 immediate wins (rAF-gate scroll + regenerate static base from real VRI polygons with lake/alpine/coast masking — fixes coastline bleed AND the 'too much green' narrative disconnect + prefers-reduced-motion); Phase 2 hero photo fades; Phase 3 scrub architecture (DECISION PENDING: shader-target vs frames-stopgap); Phase 4 gold old-growth reveal beat (DECIDED: render VRI≥250 at its TRUE % + keep 0.3%/35k ha as the big-trees subfigure; real % must be computed, not invented); Phase 5 z10+ rectangle artifact tile rebuild. Pre-flight probes before Phase 3: measure updateImage() cost, MAX_TEXTURE_SIZE on mobile, COUNT(*) province cutblocks. Still pending from before: Tier-2 conflict layer; DEFERRED relay (3.8 preset opacity + cutblocks boundary-aware timeline); 4 broken monitoring tests."
testing: null
pinned: false
shipped:
  - date: 2026-06-20
